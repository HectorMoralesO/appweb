...
if map_button: # al hacer clic en el botón
    ...
    car_data_type_condition_mean = car_data.groupby(['type', 'condition']).mean(numeric_only=True).reset_index() # Agrupar datos por las variables "type" y "condition"
    fig = px.treemap(car_data_type_condition_mean, path=[px.Constant("Tipos de carros por precio"), 'type', 'condition'], values='days_listed',
                  color='price',
